...
- [x] **M2 — Fan assistant v1**: `/chat` endpoint + chat UI, answers wayfinding +
      accessibility questions from static zone data, 3 languages
- [x] **M3 — Ops dashboard v1**: density map (Recharts + venue layout), manual density
      entry form, live Firestore listener
- [x] **M4 — Recommendation engine**: threshold check + Gemini recommendation + debounce
      cache + multilingual alert draft + approve-and-publish flow
- [x] **M5 — Incident triage**: `/incident` endpoint, server-side severity validation,
      draft response/comms UI
- [x] **M6 — Briefing generator + transport comparison**: `/briefing`,
      `/transport-compare`
- [x] **M7 — Polish + submission**: Antigravity glassmorphic pass on both views,
